<?php
/*
Plugin Name: Comic Easel (S&P Edition)
...
Version: 1.15.1
Author: Philip M. Hofer (Frumph) & Nicholas Avenell (Aquarion)
Author URI: http://frumph.net/

Copyright 2012-2018 Philip M. Hofer (Frumph)  (email : philip@frumph.net)
Copyright 2024- Nicholas Avenell (Aquarion)  (email : nicholas@aquarionics.com)
...
function ceo_initialize_post_types()
{
    ...
    if (!post_type_exists('comic')) {
        $menu_position = 5;
        if (class_exists('Jetpack_Comic')) {
            $menu_position = 6;
        } /* Allow Jetpack to have 5 */
        ...
        if (empty($comic_slug) || is_array($comic_slug)) {
            $comic_slug = 'comic';
        }
        if (ceo_pluginfo('enable_chapter_in_url')) {
            $comic_slug = $comic_slug.'/%chapters%';
        }
        ...
        if (empty($chapter_slug) || is_array($chapter_slug)) {
            $chapter_slug = 'Chapter';
        }
        if (empty($chapter_slug_plural) || is_array($chapter_slug_plural)) {
            $chapter_slug_plural = 'Chapters';
        }
...
if (is_admin()) {
    ...
    @include 'ceo-admin.php';
    @include 'functions/admin-meta.php';
} else {
